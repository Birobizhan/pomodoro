...
async def create_user(body: UserCreateBasicSchema, user_service: UserService = Depends(get_user_service)):
    return await user_service.create_user(username=body.username, password=body.password, work_duration=body.work_duration,
                                          short_break_duration=body.short_break_duration,long_break_duration= body.long_break_duration)
